...
class CourseController {
  ...
  async add(req, res) {
    try {
      ...
      const checkObj = await config.sequelize.query(
        `SELECT course_id FROM courses WHERE course_category_id = ? AND stream_id = ? AND course_name = ?  AND is_deleted=false`,
        {
  ...
  async update(req, res) {
    try {
      ...
      if (courseClasses.length) {
		const delCourseClass = `DELETE FROM course_classes WHERE course_id = ?`;
		await config.sequelize.query(delCourseClass, {
			replacements: [id],
			raw: true,
      ...
      // courseExamTypes = [{exam_type_id: 1,subExamTypes: [{sub_exam_type_id: 1},{sub_exam_type_id: 2}]}]
      if (courseExamTypes.length) {
			const delCourseExam = `DELETE FROM course_exam_types WHERE course_id = ?`;
			await config.sequelize.query(delCourseExam, {
				replacements: [id],
				raw: true,
				type: config.sequelize.QueryTypes.DELETE,
			  });
			...
			if(element.subExamTypes.length) {
			  element.subExamTypes.forEach((ele) => {
				  subinserQry += `(${id},${element.exam_type_id},${ele.sub_exam_type_id}),`;
              		...
              		subinserQry += `(${id},${element.exam_type_id},null),`;
				}
		});
