...
class SubTopicController {
  ...
  async add(req, res) {
    try {
      ...
      const checkObj = await config.sequelize.query(
        `SELECT sub_topic_id FROM sub_topics WHERE sub_topic_name = ? AND topic_id = ? AND is_deleted=false`,
        {
  ...
  async delete(req, res) {
    try {
      const { id } = req.params;
      const dataObj = await config.sequelize.query(
        `SELECT * FROM sub_topics WHERE sub_topic_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.SELECT,
      ...
      await config.sequelize.query(
        `DELETE FROM sub_topics WHERE sub_topic_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.UPDATE,
