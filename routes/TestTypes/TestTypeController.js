...
class TestTypeController {
  ...
  async add(req, res) {
    try {
      ...
      const checkObj = await config.sequelize.query(
        `SELECT test_type_id FROM test_types WHERE test_type_name = ? AND is_deleted=false`,
        {
  ...
  async delete(req, res) {
    try {
      const { id } = req.params;
      const dataObj = await config.sequelize.query(
        `SELECT test_type_id FROM test_types WHERE test_type_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.SELECT,
      ...
      await config.sequelize.query(
        `DELETE FROM test_types WHERE test_type_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.UPDATE,
