...
class ProblemActualTypeController {
  ...
  async add(req, res) {
    try {
      ...
      const checkObj = await config.sequelize.query(
        `SELECT problem_actual_type_id FROM problem_actual_types WHERE problem_type_id = ? 
        AND problem_actual_type_name = ? AND is_deleted=false`,
        {
  ...
  async delete(req, res) {
    try {
      const { id } = req.params;
      const dataObj = await config.sequelize.query(
        `SELECT problem_actual_type_id FROM problem_actual_types WHERE problem_actual_type_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.SELECT,
      ...
      await config.sequelize.query(
        `DELETE FROM problem_actual_types WHERE problem_actual_type_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.UPDATE,
