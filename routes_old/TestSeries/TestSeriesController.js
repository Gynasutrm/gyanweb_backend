...
class TestSeriesController {
  ...
  async add(req, res) {
    try {
      ...
      const checkObj = await config.sequelize.query(
        `SELECT test_series_id FROM test_series WHERE stream_id = ? AND class_id = ? AND test_type_id = ? AND test_series_name = ?`,
        {
	...
	console.log(courseCatAndCourseID.length);
    /*  if (courseCatAndCourseID.length) {
        let subExQry =
          "INSERT INTO test_series_course (test_series_id, course_category_id, course_id) VALUES ";
        courseCatAndCourseID.forEach((element) => {
          element.course_ids.forEach((ele) => {
            subExQry += `(${lastInsertedId},${element.course_category_id},${ele.course_id}),`;
          });
        });
        ...
        courseCatAndCourseID.forEach((element) => {
          element.subExamTypes.forEach((ele) => {
            if (element.test_series_id) {
              subUpdateCeQry += `UPDATE test_series_course SET course_category_id = '${element.course_category_id}',course_id = '${ele.course_id}' WHERE test_series_id = '${element.test_series_id}';`;
            } else {
              subinserQry += `(${id},${element.course_category_id},${ele.course_id}),`;
            }
          });
        });
        if (subinserQry != "") {
  ...
  async delete(req, res) {
    try {
      const { id } = req.params;
      const dataObj = await config.sequelize.query(
        `SELECT * FROM test_series WHERE test_series_id = ? AND is_deleted = false`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.SELECT,
      ...
      await config.sequelize.query(
        `UPDATE test_series SET is_deleted = true WHERE test_series_id = ?`,
        {
          replacements: [id],
          raw: true,
          type: config.sequelize.QueryTypes.UPDATE,
