...
class CommonController {
    ...
    try {          
      ...
      for (const test_type of dataObj) {        
        ...
        for(const data of seriesObj ){
          ...
          `SELECT count(*) as count FROM tests WHERE test_series_id =? AND status=True
          AND '${currentDate}' BETWEEN test_start_date AND test_end_date`,
            {
              raw: true,
  ...
  async preStartTest(req,res){
    try {      
      ...
      // check the statu of test and if it is running then calculate the running time, if over then submit 
      if(attemptId[0].test_status==3){
        return res
          .status(200)
      ...
      if(attemptId[0].test_status==2){
        const attempt_id=attemptId[0].user_test_attempt_id;
        ...
        if(countdowm_minuts>test_duration){
          ...
          const submit=await submitTest(userId,attempt_id);
          if(submit){
  ...
  async submitTest(req, res) {
    try {
      ...
      if(submit){
        ...
        return res
        .status(200)
        .json({ statusCode: 203, message: "Some error occurred while submiting your test! Please try again." });
      }
    } catch (error) {
      console.log("-error", error);
      return res
